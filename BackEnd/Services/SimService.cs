...
namespace HahnCargoSim.Services
{
    public class SimService : ISimService
    {
        ...
        public bool QueueMoveOrder(MoveOrder moveOrder)
        {
            ...
            loggerService.Log($"Move Started: {moveOrder.Owner} paid {travelCost} Coins to move transporter {moveOrder.CargoTransporterId} from {startNode.Id} to {moveOrder.TargetNodeId}.");
        ...
        public Task RunTick(int tick)
        {
            ...
            // Perform operations for each simulation tick
            UpdateMoveOrders();
            UnloadTransporters();
            LoadTransporters();
            RemoveExpiredOrders();
            ...
            // Remove completed move orders
            MoveOrderQueue.RemoveAll(moveOrder => moveOrder.Done);

            // Evaluate the need for new transporters
            EvaluateTransporterPurchase();

            return Task.CompletedTask;
        }
        private void EvaluateTransporterPurchase()
        {
            foreach (var owner in userService.GetAllUserIds())
            {
                var availableCoins = userService.GetCoinAmount(owner);
                if (availableCoins >= 1000)
                {
                    var randomNode = gridService.GetRandomNode();
                    transporterService.Buy(owner, randomNode.Id);
                }
            }
        }
